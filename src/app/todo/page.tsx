...
export default function TodoPage() {
  ...
  const handleDeleteSelected = () => {
    if (selectedTodos.size > 0) {
      if (window.confirm(`Apakah Anda yakin ingin menghapus ${selectedTodos.size} item terpilih?`)) {
    ...
    <div className="min-h-screen">
      <header className="flex justify-between items-center bg-white border-b px-8 py-3 shadow-sm">
        ...
        <div className="flex items-center space-x-2">
          ...
          <img src="/avatar.png" alt="Avatar" className="w-20 h-13 rounded-full" />
        ...
        <Card className="shadow-lg">
          <CardContent>
            <div className="flex space-x-2 mb-4 justify-center">
              ...
              <Button onClick={() => setFilterStatus('done')} variant={filterStatus === 'done' ? 'default' : 'outline'}>Done</Button>
              <Button onClick={() => setFilterStatus('undone')} variant={filterStatus === 'undone' ? 'default' : 'outline'}>Undone</Button>
            </div>
                ...
                <li key={todo.id} className="flex items-center justify-between p-2 border rounded">
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => handleToggleSelectTodo(todo.id, checked)}
                    />
                    <Label
                  ...
                  <div className="flex items-center space-x-1">
                      ...
                      <Button variant="ghost" size="icon" onClick={() => handleSingleDeleteTodo(todo.id)}>
            ...
            >
              Deleted Selected ({selectedTodos.size})
            </Button>
          </CardContent>
        </Card>
      </div>
